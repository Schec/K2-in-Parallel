...
#                calc_start = time.time()
                ...
                while (OKToProceed == True and len(pi) < u):
                    ...
                    if len(iters) > 0:
                        ...
                        for z_hat in iters:
                            f_to_max[z_hat] = f_combinations(node_order[i],pi+[z_hat],attribute_values,df, f_kernel)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description = '''K2 In Parallel using MPI and CUDA:  Calculates the parent set for each node in your data file and returns a dictionary of the form {feature: [parent set]}.''')
    ...
    parser.add_argument('--outfile', nargs='?',default=None,help='The output file where the dictionary of {feature: [parent set]} will be written')
